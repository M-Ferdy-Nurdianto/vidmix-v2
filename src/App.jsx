  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (e.message.includes('RENDER_CANCELED')) {
        toast.error('Proses render dibatalkan oleh pengguna.', { id: 'render' });
      } else {
    ...
    <div className="min-h-screen bg-[#F4F4F0] text-zinc-900 font-mono p-6 select-none relative pb-16">
              ...
              >
                ...
                {videos.length > 0 && (
                  <div className="space-y-2 mt-2">
                    {videos.map((vid, idx) => (
                      <div key={vid.id} className="flex items-center gap-2 bg-white border-2 border-black p-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                        <div className="font-black w-6 h-6 flex items-center justify-center text-xs bg-[#FFE500] border-2 border-black">{idx + 1}</div>
                        <div className="truncate flex-1 font-bold text-xs" title={vid.path}>
                          {vid.path.split('\\').pop().split('/').pop()}
                        </div>
                        <button 
      ...
      {editingVideoId && (
        <div className="fixed inset-0 z-50 bg-[#F4F4F0] text-zinc-900 font-mono flex flex-col p-6">
          {/* Header */}
          <div className="h-16 border-4 border-black bg-white flex items-center justify-between px-6 mb-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0">
            ...
            <div className="flex items-center gap-3">
              <button 
                onClick={() => { setEditingVideoId(null); setSelectedLayerId(null); }}
                className="bg-[#00FF55] hover:bg-[#00CC44] text-black border-4 border-black px-6 py-2 font-black text-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5"
              >
                SIMPAN & TUTUP EDITOR
              </button>
            </div>
          ...
          <div className="flex flex-1 min-h-0 gap-4">
            ...
            <div className="w-72 flex flex-col gap-4 overflow-y-auto pr-2 pb-2">
              {/* Tambah Element */}
              <div className="border-4 border-black bg-white p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button 
                    onClick={() => {
                      const newLayer = { id: Date.now().toString(), type: 'spectrum', name: 'Audio Spectrum', color: 'white', style: 'line', x: 50, y: 80, scale: 1, rotation: 0, zIndex: (videos.find(v => v.id === editingVideoId)?.layers.length || 0) + 1 };
                      setVideos(prev => prev.map(v => v.id === editingVideoId ? { ...v, layers: [...v.layers, newLayer] } : v));
                      setSelectedLayerId(newLayer.id);
                    }} 
              ...
              {/* Stiker / GIF Panel */}
              <div className="border-4 border-black bg-white p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0 max-h-56 flex flex-col">
                    ...
                    <div className="flex flex-col gap-3">
                      {wmLayer ? (
                        ...
                        <button 
                          ...
                          className="w-full bg-[#FFE500] hover:bg-[#E5CD00] text-black font-bold border-2 border-black py-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 text-xs"
                        >
                          + Tambah Watermark
                        </button>
                      )}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          <button 
                            ...
                            className={`p-2 text-[10px] font-bold flex items-center justify-center text-center border-2 border-black ${!wmLayer ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed opacity-60' : 'bg-zinc-100 hover:bg-[#00FF55] text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 transition-colors'}`}
                          >
                            {pos.label}
                          </button>
                        ))}
              ...
              {/* Layers List Panel */}
              <div className="border-4 border-black bg-white p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col shrink-0" style={{ minHeight: '300px' }}>
                    ...
                    return sortedLayers.map((layer, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={`w-full flex items-center gap-3 p-2 border-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-left cursor-grab active:cursor-grabbing ${isDragged ? 'opacity-50 border-dashed border-black bg-zinc-200' : isSelected ? 'bg-[#00FF55] border-black' : 'bg-white border-black hover:bg-zinc-100'} ${isDragOver ? 'border-t-4 border-t-blue-500' : ''}`}
                        >
                          ...
                          {editingLayerNameId === layer.id ? (
                            <input 
                              ...
                              className="flex-1 min-w-0 text-xs font-bold text-black bg-white border-2 border-black px-1 outline-none"
            ...
            <div className="w-80 flex flex-col pb-2">
              {selectedLayerId ? (
                <div className="h-full border-4 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-y-auto">
                  <LayerControlPanel 
                    selectedLayerId={selectedLayerId}
                    layers={videos.find(v => v.id === editingVideoId)?.layers || []}
                    setLayers={(newLayers) => {
                ...
                <div className="h-full border-4 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center text-zinc-500 p-6 text-center">
