...
export default function LayerCanvas({ mediaPath, mediaType, layers, setLayers, selectedLayerId, setSelectedLayerId }) {
  ...
  return (
    <div className="border-4 border-black bg-white p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex-1 flex flex-col justify-center">
      <div 
        ref={canvasRef}
            ...
            {layers && layers.map((layer) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected && (
                    <div 
                      className="absolute -top-16 left-1/2 transform -translate-x-1/2 flex items-center gap-1 bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-1 z-50 pointer-events-auto"
                      onPointerDown={(e) => e.stopPropagation()}
                    >
                      <button onClick={(e) => handleDuplicate(e, layer.id)} className="p-1.5 hover:bg-[#FFE500] border-2 border-transparent hover:border-black rounded-none text-black active:translate-x-0.5 active:translate-y-0.5" title="Duplicate">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                      </button>
                      <button onClick={(e) => handleReorder(e, layer.id, 'up')} className="p-1.5 hover:bg-[#00FF55] border-2 border-transparent hover:border-black rounded-none text-black active:translate-x-0.5 active:translate-y-0.5" title="Bring Forward">
                      ...
                      <button onClick={(e) => handleReorder(e, layer.id, 'down')} className="p-1.5 hover:bg-[#00FF55] border-2 border-transparent hover:border-black rounded-none text-black active:translate-x-0.5 active:translate-y-0.5" title="Send Backward">
                  ...
                  {layer.type === 'spectrum' && (
                    <div className="opacity-80 pointer-events-none relative flex items-center justify-center">
                      {layer.shape === 'circular' ? (
                        <div 
                          className={`rounded-full border-4 border-dashed ${layer.colorMode === 'rainbow_running' ? 'animate-[spin_2s_linear_infinite]' : ''}`}
                          style={{ 
                            width: '150px', 
                            height: '150px',
                            borderColor: layer.colorMode === 'rainbow_running' || layer.colorMode === 'rainbow_linear' ? 'transparent' : (layer.color || 'white'),
                            backgroundImage: layer.colorMode === 'rainbow_running' || layer.colorMode === 'rainbow_linear' ? 'conic-gradient(red, yellow, lime, aqua, blue, magenta, red)' : 'none',
                            WebkitBackgroundClip: layer.colorMode?.startsWith('rainbow') ? 'padding-box, border-box' : 'border-box',
                            backgroundOrigin: 'border-box',
                            border: layer.colorMode?.startsWith('rainbow') ? '4px dashed transparent' : undefined
                          }}
                        />
                      ) : (
                        <div 
                          className="flex items-center gap-1 overflow-hidden relative"
                          style={{ height: '80px', width: '200px', justifyContent: 'center' }}
                        >
                          {/* Rainbow background layer */}
                          {layer.colorMode?.startsWith('rainbow') && (
                            <div 
                              className={`absolute inset-0 opacity-50 ${layer.colorMode === 'rainbow_running' ? 'animate-[pulse_1s_linear_infinite]' : ''}`} 
                              style={{ 
                          ...
                          {[1, 2, 3, 4, 5, 6, 7].map((bar, idx) => {
                            return (
                              <div 
                                ...
                                style={{
                                  width: '8px',
                                  backgroundColor: layer.colorMode?.startsWith('rainbow') ? 'white' : (layer.color || 'white'),
                      ...
                      {layer.shape === 'circular' && layer.centerImage && (
